...
function compareToMedianClass(metric: string, companyValue: number | null, medianValue: number | null) {
  ...
  if (companyValue === medianValue) return 'text-slate-700 dark:text-slate-300';
...
export default function CompanyBenchmark({ ico }: { ico: string }) {
  ...
  if (loading) {
    return <div className="text-sm text-slate-700 dark:text-slate-300">Načítavam sektorové porovnanie…</div>;
  }

  if (error || !data) {
    return <div className="text-sm text-slate-700 dark:text-slate-300">Sektorové porovnanie zatiaľ nie je dostupné.</div>;
  }
        ...
        <div className="flex gap-2 text-xs">
          <button
            ...
            className={`rounded-full border px-3 py-1 ${
              geo === 'kraj'
                ? 'border-[#51c7e9]/70 bg-[#51c7e9]/20 text-slate-900 dark:text-white'
                : 'border-slate-900/10 bg-slate-900/5 text-slate-700 dark:border-white/10 dark:bg-black/20 dark:text-slate-300'
            }`}
          >
            Kraj
          </button>
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody className="divide-y divide-slate-900/10 dark:divide-white/10">
            {rows.map((r) => {
              ...
              return (
                <tr key={r.metric}>
                  <td className="py-2 pr-4 font-medium text-slate-800 dark:text-slate-200">{metricLabel(r.metric)}</td>
                  ...
                  <td className="py-2 pr-4">
                    <span
                      className={`rounded-full px-2 py-1 text-xs ${
                        cmpClass === 'text-emerald-700 dark:text-emerald-300'
                          ? 'bg-emerald-500/20 text-emerald-800 dark:text-emerald-200'
                          : cmpClass === 'text-rose-700 dark:text-rose-300'
                          ? 'bg-rose-500/20 text-rose-800 dark:text-rose-200'
                          : 'bg-slate-900/10 text-slate-700 dark:bg-white/10 dark:text-slate-300'
                      }`}
                    >
                      {cmpBadge}
                    </span>
