...
/* ---------- NEW RISK LOGIC BASED ON PERCENTILE ---------- */
...
export default async function CompanyPage({
    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm text-zinc-600">
          Neexistuje záznam pre IČO:{' '}
          <span className="font-mono">{rawIco}</span>
        </p>
  ...
  const flags = features
    ? [
        {
          k: 'negative_equity_flag',
          label: 'Negatívne vlastné imanie',
          v: features.negative_equity_flag
        },
        {
          k: 'liquidity_breach_flag',
          label: 'Problém s likviditou',
          v: features.liquidity_breach_flag
        },
        {
          k: 'high_leverage_flag',
          label: 'Vysoká zadlženosť',
          v: features.high_leverage_flag
        },
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <div className="mt-1 text-sm text-zinc-600">
              ...
              {identity.legal_form_name ? (
                <span> • {identity.legal_form_name}</span>
              ) : null}
              {identity.status ? (
            ...
            {pdLatest && typeof pdLatest.pd_pct === 'number' ? (
              <div className="min-w-[260px] rounded-xl border border-zinc-200 bg-white px-4 py-3 text-right shadow-sm">
                <div className="flex items-start justify-end gap-2">
                  <div>
                    ...
                    <div className="text-sm font-semibold">
                      {fmtPercentile(pdLatest.pd_pct)} %
                    </div>
                    <div
                      className={`text-xs font-medium ${
                        riskLabelFromPct(pdLatest.pd_pct).color
                      }`}
                    >
                  ...
                  <div className="group relative mt-0.5">
                    ...
                    <div className="pointer-events-none absolute right-0 top-6 z-10 hidden w-80 rounded-xl border border-zinc-200 bg-white p-3 text-left text-xs text-zinc-700 shadow-lg group-hover:block">
                      ...
                      <div className="mt-1 leading-relaxed">
                        Percentil vyjadruje relatívne postavenie firmy medzi
                        všetkými firmami v danom roku. Hodnota{' '}
                        {fmtPercentile(pdLatest.pd_pct)} % znamená, že firma má
                        vyššie modelové riziko než{' '}
                        {fmtPercentile(pdLatest.pd_pct)} % ostatných firiem.
                        Nejde o to, že firma má {fmtPercentile(pdLatest.pd_pct)} %
                        pravdepodobnosť bankrotu, ale že patrí medzi
                        najrizikovejšie subjekty podľa modelu.
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-2 text-xs text-zinc-500">
                  ...
                  <span className="font-medium text-zinc-700">
                    {fmtProb(pdLatest.pd_12m)}
                  </span>
                </div>

                <div className="mt-1 text-[11px] leading-relaxed text-zinc-500">
                  Ide o štatistický odhad založený na historických dátach.
                  Vzhľadom na nízku mieru bankrotov v populácii bývajú tieto
                  hodnoty prirodzene nízke.
                </div>
              </div>
            ) : null}
          </div>
